...
bookdatabase.prepare(`CREATE TABLE IF NOT EXISTS books (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, author TEXT, description TEXT, added_by DATE, FOREIGN KEY book_id REFERENCES reviews(id), FOREIGN KEY user_id REFERENCES users(id))`).run();
export const getBooks =() => bookdatabase.prepare(`SELECT * FROM books`).all();
export const getBook =()=> bookdatabase.prepare(`SELECT * FROM books WHERE id=?`).get(id);
export const saveBook=(title, author, description, added_by)=> bookdatabase.prepare(`INSERT INTO books (title, author, desciption, added_by) VALUES (?,?)`).run(title, author, description, added_by);
