...
database.prepare(`CREATE TABLE IF NOT EXISTS users
    (id INTEGER PRIMARY KEY AUTOINCREMENT, username TEXT, email TEXT, password_hashed TEXT, FOREIGN KEY book_id REFERENCES reviews(id), FOREIGN KEY user_id REFERENCES reviews(id))`).run();
...
export const saveUser=(username, email, password_hashed)=> database.prepare(`INSERT INTO users
    (username, email) VALUES (?,?)`).run(username, email, password_hashed);
